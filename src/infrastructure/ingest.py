...
from src.infrastructure.repository import upsert_competitions, upsert_clubs, upsert_swimmers, upsert_events, delete_results_for, insert_results
...
def ingest_file(conn, path: str | Path) -> int:
    ...
    comp_row = {
        "id_cpt": id_cpt, "name": doc.get("name"), "city": doc.get("city"),
        "country": doc.get("country"), "date_start": _date(doc.get("date_start")),
        "date_end": _date(doc.get("date_end")), "pool_size": doc.get("pool_size"),
        "level": doc.get("level"), "type": doc.get("type"),
        "type_code": doc.get("type_code"), "category": doc.get("category"),
        "season": doc.get("season"), "scraped_at": _dt(doc.get("scraped_at")),
    }
    ...
    for r in doc.get("results", []):
        ...
        events.setdefault(ev_key, {"distance": distance, "stroke": stroke,
                                   "is_relay": is_relay, "label": r.get("event")})

        iuf = r.get("iuf")
        if iuf is not None:
            last, first = split_name(r.get("swimmer"))
            swimmers[iuf] = {"iuf": iuf, "last_name": last, "first_name": first,
                             "full_name": r.get("swimmer"), "birth_year": r.get("birth_year"),
        ...
        staged.append({
            "ev_key": ev_key, "iuf": iuf, "id_club": id_club,
            "id_res": r.get("id_result"), "rank": r.get("rank"),
            "time_cs": r.get("time_cs"), "reaction": parse_reaction(r.get("reaction")),
            "points": r.get("points"), "phase": r.get("phase"),
            "gender": r.get("gender"), "race_date": _date(r.get("date")),
            "is_relay": is_relay,
        })
    ...
    with conn.begin():  # transaction par compétition
        ...
        rows = [{
            "id_cpt": id_cpt, "id_event": event_map[s["ev_key"]], "iuf": s["iuf"],
            "id_club": s["id_club"], "id_res": s["id_res"], "rank": s["rank"],
            "time_cs": s["time_cs"], "reaction": s["reaction"], "points": s["points"],
            "phase": s["phase"], "gender": s["gender"], "race_date": s["race_date"],
            "is_relay": s["is_relay"],
        } for s in staged]
        insert_results(conn, rows)

    log.info("cpt %s : %d résultats chargés (%s)", id_cpt, len(staged), doc.get("name") or "")
    return len(staged)
